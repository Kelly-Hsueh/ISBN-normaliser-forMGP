...
- 编辑摘要由机器人根据实际改动类型逐页自动拼接，各部分以全角分号（`；`）连接：
  ...
  | ISBN **数值本身**的格式发生了实际变化 | `根据 ISO 2108:2017（…）自动调整ISBN（…）` |

  最后一条注意：`ISBN-10 → ISBN-13 转换` 或普通连字符规范化必然伴随数值格式变化，因此会始终触发该说明；但 BookSources 链接替换或 ISBNT 合并有时只是纯结构性操作（换链接为模板、换模板名），底层 ISBN 数值原本就已是正确格式，这种情况下不会附加该说明，避免摘要显得文不对题。
